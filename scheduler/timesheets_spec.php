<?php #timesheets_spec.php
?>
	<title>Generate Specific Timesheets</title>

<?php
date_default_timezone_set('America/Denver');
require_once ('/home/teulberg/lpl-repository.com/mysql_connect_sched.php');
...
$query5 = "SELECT employee_number, last_name, first_name, exempt_status
	FROM employees WHERE active='Active' and (employee_lastday > '$startdate' or employee_lastday is null) ORDER BY last_name";
...
foreach ($array as $key=>$date){
	...
	$filename = "timesheetsspec/day$key.csv";
	...
	$query3 = "SELECT employees.employee_number, last_name, first_name, shift_start, shift_end
		...
		and (employees.employee_lastday > '$startdate' or employees.employee_lastday is null)
		ORDER BY last_name";
...
$file = "/home/teulberg/lpl-repository.com/scheduler/timesheetsspec/employees.csv";	
$handle = fopen($file, "w");
...
?>
<form action="/scheduler/timesheets_spec" method="post" name="timesheets_spec" id="timesheets_spec">
<input type="text" name="date" placeholder='XXXX-XX-XX format date'/><br/>
Enter date in timesheet range
